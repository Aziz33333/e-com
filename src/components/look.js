...
const AppLook = () => {
  ...
  // Fonction pour répéter les images pour remplir le carousel
  ...
  // Répétition des images pour remplir le carousel
  const repeatedImages = repeatImages(images, 3); // Répéter les images 3 fois pour remplir le carousel
  
  // Fonction pour diviser les images en groupes de 6
  const groupImages = (arr, size) => {
    return arr.reduce((acc, _, i) => (i % size ? acc : [...acc, arr.slice(i, i + size)]), []);
  };

  // Division des images en groupes de 6
  const imageGroups = groupImages(repeatedImages, 6);

  // Fonction pour changer l'image survolée
  const handleImageHover = (src) => {
    setHoveredImage(src);
  };

  return (
    <Container fluid className="mt-3"> {/* Utilisation de fluid pour occuper toute la largeur de l'écran */}
      <h1 style={{ marginBottom: '30px' }}>Cela peut vous intéresser</h1>
      <div style={{ position: 'relative' }}>
        ...
        >
          {imageGroups.map((group, index) => (
            <Carousel.Item key={index}>
              <Row className="gx-2">
                {group.map((image) => (
                  <Col xs={2} key={image.id}> {/* Utilisation de xs=2 pour que chaque colonne occupe 1/6 de la largeur */}
                    <div style={{ cursor: 'pointer' }}>
                      <img
                        className="d-block w-100"
                        src={hoveredImage === image.src ? 'https://images.zen.com.tn/medias/fx_folder_08_01_2024/logo_zen_bc479d6070.png' : image.src}
                        alt={image.alt}
                        style={{ height: '300px' }} // Garder la hauteur constante
        ...
        {/* Les contrôles du carousel restent inchangés */}
        <div
